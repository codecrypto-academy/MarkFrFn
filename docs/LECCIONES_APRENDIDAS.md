...
## 12. P04 — Escrow DApp (swap de tokens ERC20)

### Patrón escrow en tres fases
El flujo de un escrow de tokens ERC20 sigue siempre tres funciones:

```
createOperation  → transferFrom(creator → contrato, amountA)
completeOperation → transferFrom(completer → creator, amountB) + transfer(contrato → completer, amountA)
cancelOperation  → transfer(contrato → creator, amountA)
```

### nonReentrant obligatorio en complete y cancel
Ambas funciones (`completeOperation` y `cancelOperation`) hacen transferencias ERC20. Siempre aplicar `nonReentrant` para proteger contra ataques de reentrada con tokens maliciosos:

```solidity
function completeOperation(uint256 id) external nonReentrant operationActive(id) { ... }
function cancelOperation(uint256 id)   external nonReentrant operationActive(id) { ... }
```

### IDs de operación desde 1, no desde 0
Si `operationCount` empieza en 0 y se incrementa **antes** de crear la operación, las operaciones van de 1 a N. El índice 0 queda sin usar como centinela:

```solidity
operationCount++;
_operations[operationCount] = Operation({ id: operationCount, ... });
```

`getAllOperations()` debe iterar desde 1:

```solidity
Operation[] memory ops = new Operation[](operationCount);
for (uint256 i = 1; i <= operationCount; i++) {
    ops[i - 1] = _operations[i];
}
```

### Custom errors con parámetros para mejor DX
En el frontend se puede decodificar el selector de 4 bytes para identificar el error exacto:
...
### Allowlist de tokens: `allowedTokens` + `_tokenList`
Mantener un mapping para consultas O(1) y un array para iteración/UI:
...
### ERC20 approve en el frontend antes de operaciones de contrato
Siempre hacer `approve` y esperar confirmación antes de llamar al contrato:

```typescript
// Crear operación:
const approveTx = await tokenA.approve(ESCROW_ADDRESS, amountA);
await approveTx.wait();
const createTx = await escrow.createOperation(tokenA, tokenB, amountA, amountB);
await createTx.wait();

// Completar operación:
const approveTx = await tokenB.approve(ESCROW_ADDRESS, amountB);
await approveTx.wait();
const completeTx = await escrow.completeOperation(id);
...
## 13. Checklist para el próximo proyecto dApp
...
- [ ] Usar `nonReentrant` en todas las funciones que hagan transferencias ERC20
- [ ] Mover todos los MDs adicionales a `docs/` al crear la práctica
